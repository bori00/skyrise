...
class TpchPqpGenerator : public AbstractCompiler {
 ...
 private:
  ...
  PipelineData GenerateQ5Pipeline1(const std::vector<ObjectReference>& region_input_objects,
                                   const std::vector<ObjectReference>& nation_input_objects,
                                   const std::vector<ObjectReference>& supplier_input_objects) const;
  // Partition lineitems.
  PipelineData GenerateQ5Pipeline2(const std::vector<ObjectReference>& input_objects) const;
  // Join suppliers with lineitems.
  PipelineData GenerateQ5Pipeline3(const std::vector<ObjectReference>& supplier_input_objects,
  ...
  PipelineData GenerateQ5Pipeline4(const std::vector<ObjectReference>& input_objects) const;
  // Join orders with lineitems.
  PipelineData GenerateQ5Pipeline5(const std::vector<ObjectReference>& supplier_input_objects,
                                   const std::vector<ObjectReference>& lineitem_input_objects) const;
  // Scan and partition customers.
  PipelineData GenerateQ5Pipeline6(const std::vector<ObjectReference>& input_objects) const;
  // Join customers with lineitems.
  PipelineData GenerateQ5Pipeline7(const std::vector<ObjectReference>& customer_input_objects,
                                   const std::vector<ObjectReference>& lineitem_input_objects,
